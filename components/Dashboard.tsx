...
import { AppState, TabType } from '../types.ts';
...
const Dashboard: React.FC<DashboardProps> = ({ state, updateState, setActiveTab }) => {
  ...
  const stats = useMemo(() => {
    // Proteção contra arrays nulos ou indefinidos
    const tasks = state.tasks || [];
    const waterHistory = state.waterHistory || [];
    const studySubjects = state.studySubjects || [];
    const musicSessions = state.musicSessions || [];

    const todayTasks = tasks.filter(t => t.completed && t.date === today).length;
    const totalTodayTasks = tasks.filter(t => t.date === today).length;
    const waterToday = waterHistory.find(w => w.date === today)?.amount || 0;
    const studyHours = studySubjects.reduce((acc, s) => acc + (s.totalHours || 0), 0);
    const musicSessionsCount = musicSessions.length;
  ...
  useEffect(() => {
    const fetchQuote = async () => {
      try {
        const fetchedQuote = await getMotivationalQuote(state.user?.name || "Explorador");
  ...
  useEffect(() => {
    ...
    if (displayProgress < target) {
      const timer = setTimeout(() => setDisplayProgress(prev => Math.min(prev + 1, target)), 10);
      return () => clearTimeout(timer);
    } else if (displayProgress > target) {
      const timer = setTimeout(() => setDisplayProgress(prev => Math.max(prev - 1, target)), 10);
      return () => clearTimeout(timer);
    }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-5">
          {state.user?.avatarUrl ? (
            <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-primary-500 shadow-lg bg-white">
          ...
          <div>
            <h2 className="text-3xl font-black text-slate-800 dark:text-white uppercase tracking-tighter">
              {getGreeting()}, <span className="text-primary-500">{state.user?.name || "Explorador"}</span>!
            </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <DashboardCard icon="💧" title="Água" value={`${stats.waterAmount}ml`} subtitle="Hoje" color="bg-blue-500" onClick={() => setActiveTab('water')} />
              ...
              <BarChart data={chartData}>
                ...
                <Bar dataKey="amount" radius={[12, 12, 12, 12]} barSize={28}>
                  {chartData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={entry.amount >= (state.user?.waterGoal || 2000) ? '#0ea5e9' : '#e2e8f0'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
